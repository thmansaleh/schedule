...
export default function opperation(){
...
setDuty([...duty])
    ...
    return <div className="relative bg-green-100  overflow-x-auto ">
    <div className="w-full py-3 text-lg text-black text font-semibold text text-center ">أفراد عمليات الشرطة  </div>
       ...
       {duty.map((e,operationIndex)=>{
        ...
        <select onChange={(e)=>addOperation(e.target.value,operationIndex)} className=" opacity-0 absolute top-0 left-0 bottom-0 right-0">
           ...
           {operations.map((e,i)=>{
            ...
            if(e.isAvilable&&e.status=='true') return <option className="text" key={i} value={i}>{e.name}</option>
        ...
        <th scope="row" className="px-6 py-6 text text-center font-medium  whitespace-nowrap  ">
          {e.duty}
        </th>
